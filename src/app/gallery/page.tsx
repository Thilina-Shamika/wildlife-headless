...
export default async function GalleryPage() {
  // Fetch the gallery page data
  let pageData: any = null;
  try {
    const res = await fetch('http://ceylonicus.local/wp-json/wp/v2/pages?slug=gallery', { cache: 'no-store' });
  ...
  const fallback = {
    cover_image: {
      url: 'http://ceylonicus.local/wp-content/uploads/2025/05/gallery-cover.jpg',
      alt: 'Gallery Cover',
      width: 1920,
      height: 1284,
    },
  ...
  const galleryImages = pageData && Array.isArray(pageData.gallery) ? pageData.gallery : fallback.gallery;
  ...
  return (
    <main className="min-h-screen bg-[#f9fafb]">
      ...
      {/* Gallery Masonry Grid (Client Component) */}
